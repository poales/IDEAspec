Public Class ChrisBlasterIO
    ...
    Public Declare Sub getErrorMessage Lib "chrisblasterhost.dll" (ByVal msgbuff As String, ByVal length As Integer)
    Public Declare Function initDrivers Lib "chrisblasterhost.dll" () As Integer
    Public Declare Function openChrisBlasterConnection Lib "chrisblasterhost.dll" (ByVal name As String, ByRef devpointer As IntPtr) As Integer
    Public Declare Function closeChrisBlasterConnection Lib "chrisblasterhost.dll" (ByVal devpointer As IntPtr) As Integer

    Public Declare Function getClockFrequency Lib "chrisblasterhost.dll" (ByVal devpointer As IntPtr) As Integer
    Public Declare Function getSlowIOlimits Lib "chrisblasterhost.dll" (ByRef numouts As Integer, ByRef numins As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function getBoardStatus Lib "chrisblasterhost.dll" (ByRef bstat As Byte, ByVal devpointer As IntPtr) As Integer
    Public Declare Function castIntToByte Lib "chrisblasterhost.dll" (ByVal input As Integer) As Byte

    Public Declare Function allocateNewProtocol Lib "chrisblasterhost.dll" (ByVal pindex As Integer, ByVal numloops As Integer, ByVal numbitmasks As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function repeatLoop Lib "chrisblasterhost.dll" (ByVal pindex As Integer, ByVal lindex As Integer, ByVal numreps As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function addBitmask Lib "chrisblasterhost.dll" (ByVal pindex As Integer, ByVal lindex As Integer, ByVal bitmask As UInteger, ByVal cycles As UInteger, ByVal devpointer As IntPtr) As Integer
    Public Declare Function setTerminalBitmask Lib "chrisblasterhost.dll" (ByVal pindex As Integer, ByVal bitmask As UInteger, ByVal devpointer As IntPtr) As Integer
    Public Declare Function freeMemory Lib "chrisblasterhost.dll" (ByVal pindex As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function runProtocol Lib "chrisblasterhost.dll" (ByVal pindex As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function startProtocol Lib "chrisblasterhost.dll" (ByVal pindex As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function checkIfDone Lib "chrisblasterhost.dll" (ByVal devpointer As IntPtr) As Integer
    Public Declare Function killProtocol Lib "chrisblasterhost.dll" (ByVal devpointer As IntPtr) As Integer
    Public Declare Function changeOutputBitmask Lib "chrisblasterhost.dll" (ByVal bitmask As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function slowIOreadByte Lib "chrisblasterhost.dll" (ByVal address As Byte, ByRef getbyte As Byte, ByVal devpointer As IntPtr) As Integer
    Public Declare Function slowIOwriteByte Lib "chrisblasterhost.dll" (ByVal address As Byte, ByVal setbyte As Byte, ByVal devpointer As IntPtr) As Integer
    Public Declare Function refreshBoard Lib "chrisblasterhost.dll" (ByVal devpointer As IntPtr) As Integer

    Public Declare Function sendCommand Lib "chrisblasterhost.dll" (ByVal command As Byte, ByVal devpointer As IntPtr) As Integer
    Public Declare Function sendCommandWithParameters Lib "chrisblasterhost.dll" (ByVal command As Byte, ByVal p1 As Integer, ByVal p2 As Integer, ByVal p3 As Integer, ByVal p4 As Integer, ByVal devpointer As IntPtr) As Integer
    Public Declare Function writeInt32 Lib "chrisblasterhost.dll" (ByVal number As UInteger, ByVal startindex As Byte, ByVal devpointer As IntPtr) As Integer
    Public Declare Function readInt32 Lib "chrisblasterhost.dll" (ByRef number As UInteger, ByVal startindex As Byte, ByVal devpointer As IntPtr) As Integer
    ...
    Public Sub New(ByVal board_name As String)
        ...
        If (funcstat = 0) Then 'failed to initialize drivers
            MessageBox.Show("Unable to initialize Adept drivers by Digilent Inc..")
            ...
            If (board_name <> "") Then
                ...
                Else
                    ...
                    If (DiagnosticMode) Then
                        MessageBox.Show("Unable to establish connection to Digilent board named " + name + ".")
                        System.Windows.Forms.Application.DoEvents()
                    End If
                End If
    ...
    Public Overridable Function CreateNewProtocol(ByVal protocol_number As Integer, ByVal number_loops As Integer) As Boolean
        If (protocol_number > 255 Or protocol_number < 0) Then
            If (DiagnosticMode) Then
                MessageBox.Show("Protocol index " + protocol_number + " is too high")
                System.Windows.Forms.Application.DoEvents()
                Return False
            End If
        End If
    ...
    Public Overridable Function GenerateTimeMap(ByRef the_protocol As Protocol) As Double()
        ...
        Return times
    End Function
    Public Overridable Function ProgramProtocol(ByVal index As Integer, ByRef the_protocol As Protocol, ByRef time_map() As Double) As Boolean
        ...
        If (Not connected) Then
            ...
            Return False
        End If
        CreateNewProtocol(index, the_protocol.Number_Loops, (10 * the_protocol.Number_Measuring_Lights)) ' 10 bitmasks per measuring light should be more than enough, as it should only need 3 + 5 * (number of lights)
        ...
        For p_loop As Integer = 0 To the_protocol.Number_Loops - 1 ' p_loop is loop number
            ' pre-pulse (if set)
            ...
            End If
            For p_light As Integer = 0 To the_protocol.Number_Measuring_Lights - 1
                If the_protocol.Take_Data(p_loop) > 0 Then
                    ' measureing interval
                    success = success And AddNextBitmask(index, p_loop, _
                               makeBitmask(the_protocol.Intensity(p_loop), 0, 0, the_protocol.Primary_Gain(p_light), _
                                           the_protocol.Reference_Gain(p_light), the_protocol.Far_Red(p_loop), _
                                           the_protocol.Saturating_Pulse(p_loop), the_protocol.Blue_Actinic(p_loop), _
                                           0, 0), _
                    ...
                    ' measuring pulse
                    success = success And AddNextBitmask(index, p_loop, _
                               makeBitmask(the_protocol.Intensity(p_loop), 0, the_protocol.Measuring_Light(p_light), _
                                           the_protocol.Primary_Gain(p_light), _
                                           the_protocol.Reference_Gain(p_light), the_protocol.Far_Red(p_loop), _
                                           the_protocol.Saturating_Pulse(p_loop), the_protocol.Blue_Actinic(p_loop), _
                                           0, 1), _
                               Protocol.TimeInSeconds(the_protocol.Measuring_Pulse_Duration))
                    ...
                    ' ADC read time
                    success = success And AddNextBitmask(index, p_loop, _
                               makeBitmask(the_protocol.Intensity(p_loop), 1, the_protocol.Measuring_Light(p_light), _
                                           the_protocol.Primary_Gain(p_light), _
                                           the_protocol.Reference_Gain(p_light), the_protocol.Far_Red(p_loop), _
                                           the_protocol.Saturating_Pulse(p_loop), the_protocol.Blue_Actinic(p_loop), _
                                           0, 0), _
                ...
                Else ' not taking data, so the measuring light and trigger do not fire
                    ...
                    ' measuring pulse
                    success = success And AddNextBitmask(index, p_loop, _
                               makeBitmask(the_protocol.Intensity(p_loop), 0, 0, _
                                           the_protocol.Primary_Gain(p_light), _
                                           the_protocol.Reference_Gain(p_light), the_protocol.Far_Red(p_loop), _
                                           the_protocol.Saturating_Pulse(p_loop), the_protocol.Blue_Actinic(p_loop), _
                                           0, 0), _
        ...
        success = success And SetPostProtocolBitmask(index, makeBitmask(the_protocol.Intensity(the_protocol.Number_Loops - 1), _
                                                  0, 0, 0, 0, the_protocol.Far_Red(the_protocol.Number_Loops - 1), _
    ...
    Public Overridable Function makeBitmask(ByVal actinic_intensity As Short, ByVal ADC_trigger As Integer, _
        ...
        'byte1 = dddccccb (little endian) where b = adc trigger, c = measuring light, and d = sample gain
        byte1 = &H0
        ...
        If (farred > 0) Then
            byte2 = byte2 + 8
        End If
        If (saturation_pulse > 0 Or actinic_intensity = 256) Then
            byte2 = byte2 + 16
        End If
