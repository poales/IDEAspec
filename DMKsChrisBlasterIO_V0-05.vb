Public Class ChrisBlasterIO
    ...
    Public Overridable Function ProgramProtocol(ByVal index As Integer, ByRef the_protocol As Protocol, ByRef time_map() As Double) As Boolean
        ...
        Dim temp_gain As Integer
        inject = 1
        Dim test As Single
        Dim takeoffset As Integer = False
        ...
        CreateNewProtocol(index, the_protocol.Number_Loops, (5 * the_protocol.Number_Loops * the_protocol.Number_Measuring_Lights)) ' 20 bitmasks per measuring light should be more than enough, as it should only need 3 + 5 * (number of lights)
        ...
        For p_loop As Integer = 0 To the_protocol.Number_Loops - 1 ' p_loop is loop number
            ...
            ' i am using the xe flash signal to gate the infinite sample and hold
            ...
            If (the_protocol.Xe_Flash(p_loop) > 0) Then
                ...
                success = success And AddNextBitmask(index, p_loop, _
                                                       makeBitmask(intensity_holder, 0, the_protocol.Measuring_Light(0), _
                                                                   ...
                                                                   the_protocol.Saturating_Pulse(p_loop), the_protocol.Blue_Actinic(p_loop), _
                                                                1, 0), _
                                                                 Protocol.TimeInSeconds(the_protocol.Measuring_Pulse_Duration(0 + 1)))
            ...
            For p_light As Integer = 0 To the_protocol.Number_Measuring_Lights - 1
                If the_protocol.Take_Data(p_loop) > 0 Then
                    ...
                    If inject = 1 Then
                        success = success And AddNextBitmask(index, p_loop, _
                                   makeBitmask(intensity_holder, 0, 0, the_protocol.Primary_Gain(p_light), _
                                               0, the_protocol.Far_Red(p_loop), _
                    ...
                    End If
                    test = Protocol.TimeInSeconds(the_protocol.Measuring_Pulse_Duration(p_light)) ' changed from p_light+1
                    ' measuring pulse
                    If inject = 1 Then
                        For pulsepulse = 1 To 1
                            success = success And AddNextBitmask(index, p_loop, _
                                                   makeBitmask(intensity_holder, 0, the_protocol.Measuring_Light(p_light), _
                                                               ...
                                                               the_protocol.Saturating_Pulse(p_loop), the_protocol.Blue_Actinic(p_loop), _
                                                            1, 0), _
                                                             Protocol.TimeInSeconds(the_protocol.Measuring_Pulse_Duration(p_light))) ' changed from p_light+1
                    ...
                    Else
                        ...
                        success = success And AddNextBitmask(index, p_loop, _
                                   makeBitmask(intensity_holder, 0, the_protocol.Measuring_Light(p_light), _
                                               ...
                                               the_protocol.Saturating_Pulse(p_loop), the_protocol.Blue_Actinic(p_loop), _
                                               0, 1), _
                                   Protocol.TimeInSeconds(the_protocol.Measuring_Pulse_Duration(p_light + 1)))
                    End If
